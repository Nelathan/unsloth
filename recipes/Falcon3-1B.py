...
finetome = finetome.shuffle(seed=42)
finetome = finetome.select(range(2000))
finetome = standardize_sharegpt(finetome)
print(finetome[0])

stheno = load_dataset("anthracite-org/stheno-filtered-v1.1", split = "train")
stheno = stheno.shuffle(seed=42)
stheno = stheno.select(range(2000))
stheno = standardize_sharegpt(stheno)
print(stheno[0])

dataset = concatenate_datasets([finetome, stheno])
...
messages = [
    { "role": "system", "content": "The following is a conversation between a State of the Art, innovative Tsundere AI assistant, and a user." },
    { "role": "user", "content": "Well I have a task I could use some help with but it sounds like you don't really want to help me..." }
]
