...
from transformers import logging, PreTrainedTokenizerFast, AutoConfig
...
model_config = AutoConfig.from_pretrained(model_input)
...
# tokenizer = PreTrainedTokenizerFast.from_pretrained("/workspace/recipes/Falcon3-tokenizer", padding_side="left")
...
args = SFTConfig(
    output_dir = f"outputs/{product}/{timetamp}",
    run_name = f"{timetamp}",
    report_to = "wandb",
    bf16 = True,
...
print("test synthetic roleplay");
print_test_inference(test_conversations);

test_conversations = sugarquill.shuffle(seed=42).select(range(5)).map(lambda row: {
